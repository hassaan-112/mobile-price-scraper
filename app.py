...
def scrap():
    ...
    time.sleep(1)
    placeholder.write("writing data to excel file")
    time.sleep(1)
...
if os.path.exists("mobiles.csv"):
    ...
    price = st.number_input("price less than")  # price filter
    if price:
    ...
    for i in brands:
        ...
        aveg = int(total_price / count)
        avg_price[i] = aveg

    select_list = [f"{i} - Average price: {avg_price[i]}" for i in brands]
    selectedBrand = st.selectbox("Filter by mobile", ['All'] + select_list)
    
    if selectedBrand != "All":
        selected_brand = selectedBrand.split(" -")[0]
        search_filter = search_filter[search_filter['Brand'] == selected_brand]
    
    st.dataframe(search_filter, use_container_width=True)
else:
    placeholder.text("Data does not exist")
